...
public class StatelessTrieLogFactory implements TrieLogFactory {

  private static final Logger LOG = LoggerFactory.getLogger(StatelessTrieLogFactory.class);

  public StatelessTrieLogFactory() {}
  ...
  public TrieLog create(final TrieLogAccumulator accumulator, final BlockHeader blockHeader) {
    ...
    LOG.info(
        "creating Stateless TrieLog for block {}:{}",
  ...
  public static PluginTrieLogLayer readFrom(final RLPInput input) {
    ...
    while (!input.isEndOfCurrentList()) {
      ...
      if (input.nextIsNull()) {
        ...
        input.leaveList();
        code.put(address, new PathBasedValue<>(oldCode, newCode, isCleared));
      }
      ...
      if (input.nextIsNull()) {
        ...
        input.enterList();
        final AccountValue oldValue = nullOrValue(input, PmtStateTrieAccountValue::readFrom);
        final AccountValue newValue = nullOrValue(input, PmtStateTrieAccountValue::readFrom);
        final boolean isCleared = getOptionalIsCleared(input);
        input.leaveList();
        accounts.put(address, new PathBasedValue<>(oldValue, newValue, isCleared));
      }
  ...
  public static <T> void writeInnerRlp(
    ...
    if (value != null && !value.isLastStepCleared()) {
      output.writeNull();
    } else {
      output.writeInt(1);
    }
